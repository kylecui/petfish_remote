...
if (command === 'start') {
  ...
  if (!configPath) {
    console.error('Usage: connector start <config.yaml>');
    process.exit(1);
  }
  daemonStart(configPath);
  ...
  if (!configPath) {
    console.error('Usage: connector --supervisor <config.yaml>');
    process.exit(1);
  }
  const thisScript = fileURLToPath(import.meta.url);
  supervisorRun(thisScript, configPath);
} else {
  const configPath = command === '--worker' ? process.argv[3] : command;
  runConnector(configPath);
}

function runConnector(configPath: string | undefined) {
  let config: ReturnType<typeof loadConnectorConfig>;
  try {
